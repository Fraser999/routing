...
//             .iter()
//             .find(|&(_, table)| match table.should_split() {
//                 Some(_) => true,
//                 None => false,
//             })
//             .map(|(&node, _)| node) {
...
//         while let Some(node) = self.nodes
//             .iter()
//             .find(|&(_, table)| match table.should_merge() {
//                 Some(_) => true,
//                 None => false,
//             })
//             .map(|(&node, _)| node) {
...
//                 Some(nodes) => {
//                     // TODO: if the node in query itself is part of the close members, there is
//                     //       chance the group doesn't
//                     //       got split yet from its perspective
//                     //       execute this test with seed of [2817869685, 3020312298, 343512997,
//                     //       915190025]
...
//                     //       which missed itself 7282345220160771589 and the group is not split
//                     //       yet.
//                     // Also, `close_nodes` returns the closest known group to the target from our
//                     // perspective, meanwhile here we are only checking against the
//                     // `closest_group` assert_eq!(group, nodes);
